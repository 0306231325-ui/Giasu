...
function FormThemHoSo({ hoSo, danhMuc, dangTaiDanhMuc }) {
    ...
    const moFileDangChon = () => {
        if (!hoSo.form.tai_lieu) return;

        setTaiLieuDangXem({
            urlTrucTiep: URL.createObjectURL(hoSo.form.tai_lieu),
            tenFile: hoSo.form.tai_lieu.name,
            tieuDe: "File minh chứng đã chọn",
            laUrlTam: true,
        });
    };

    const dongFileDangChon = () => {
            ...
            >
                <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white text-slate-900 shadow-2xl">
                    <div className="flex items-start justify-between border-b border-slate-100 px-6 py-5">
                        ...
                        <button
                            type="button"
                            onClick={hoSo.dongForm}
                            aria-label="Đóng"
                            className="rounded-xl p-2 text-2xl text-slate-400 hover:bg-slate-100"
                        >
                            ×
                        </button>
                    </div>
                    ...
                    <div className="grid gap-5 p-6 md:grid-cols-2">
                        ...
                        <label className={LOP_NHAN}>
                            Loại tài liệu<DauBatBuoc />
                            <select
                                className={LOP_INPUT}
                                name="loai_bang"
                                value={hoSo.form.loai_bang}
                                onChange={hoSo.thayDoi}
                            >
                            ...
                            <div className="flex flex-col gap-3 rounded-xl border border-blue-100 bg-blue-50/70 p-4 md:col-span-2 sm:flex-row sm:items-center sm:justify-between">
                                <div className="min-w-0">
                                    ...
                                    <p className="mt-1 truncate text-sm font-bold text-slate-800">
                                        {hoSo.form.tai_lieu.name}
                                    </p>
                                </div>
                                <button
                                    type="button"
                                    onClick={moFileDangChon}
