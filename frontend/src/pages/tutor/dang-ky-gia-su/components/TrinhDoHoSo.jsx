                                ...
                                <div key={muc.id} className="rounded-xl border border-slate-200 bg-white p-4">
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="min-w-0">
                                            <p className="truncate text-sm font-bold">{muc.ten_bang}</p>
                                            <p className="mt-1 text-xs text-slate-500">{[muc.chuyen_nganh, muc.truong_don_vi].filter(Boolean).join(" · ")}</p>
                                            <p className="mt-2 inline-flex rounded-full bg-blue-50 px-2.5 py-1 text-[11px] font-bold text-blue-700">
                                                {layTenTrinhDo(muc.trinh_do_giasu_id)}
                                            </p>
                                            <p className="mt-2 truncate text-xs font-semibold text-blue-600">{muc.tai_lieu.name}</p>
                                        </div>
